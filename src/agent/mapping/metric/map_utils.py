...
def get_local_map_boundaries(global_loc: torch.IntTensor,
    ...
    if p.global_downscaling > 1:
        ...
        if y2 > p.global_map_size:
            y1 = torch.tensor(p.global_map_size - p.local_map_size, device=device, dtype=dtype)
        ...
        if x2 > p.global_map_size:
            x1 = torch.tensor(p.global_map_size - p.local_map_size, device=device, dtype=dtype)

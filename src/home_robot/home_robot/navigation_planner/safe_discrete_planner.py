...
class SafeDiscretePlanner:
    ...
    def __init__(
        ...
        safe_obs_dilation_selem_radius: int,
        safe_dist_thresh: int,
        ...
        self.safe_obs_dilation_selem_radius = safe_obs_dilation_selem_radius
    ...
    def reset(self):
        ...
        self.safe_obs_dilation_selem = skimage.morphology.disk(
            self.safe_obs_dilation_selem_radius
        )
    ...
    def plan(
        ...
        ##########################################################################
        (
            short_term_goal_safe,
            closest_goal_map_safe,
            replan_safe,
            stop_safe,
            goal_pt_safe,
            d2g_cm_safe,
        ...
        ##########################################################################
        # Combine both planning strategies
        # If safe d2g is similar to regular d2g, pick safe plan.
        ##########################################################################
        use_safe_plan = False
        if not (replan_reg or replan_safe or stop_safe or stop_reg):
            # Planning succeeded
            deviation = 100.0 * abs(d2g_cm_safe - d2g_cm_reg) / d2g_cm_reg
            print("\n plan deviation: {:.3f} \n".format(deviation))
            if deviation <= self.safe_dist_thresh:
                # safe d2g is similar to regular d2g
                use_safe_plan = True
        if use_safe_plan:
            short_term_goal = short_term_goal_safe
            closest_goal_map = closest_goal_map_safe
            replan = replan_safe
            stop = stop_safe
        else:
            short_term_goal = short_term_goal_reg
            closest_goal_map = closest_goal_map_reg
            replan = replan_reg
            stop = stop_reg
        ...
        if debug:
            ...
            print("Replan:", replan)
            print("Safe planning:", use_safe_plan)
        # t1 = time.time()
    ...
    def _get_short_term_goal(
        ...
        if safe:
            dilated_obstacles = cv2.dilate(
                obstacles, self.safe_obs_dilation_selem, iterations=1
            )
        else:
            dilated_obstacles = cv2.dilate(
        ...
        # Measure planned distance to goal from agent
        d2g = planner.get_distance_to_state(state) * self.map_resolution
        ...
        if not np.any((planner.goal_map > 0.5) & (planner.traversible > 0.5)):
            ...
            replan = True
        return short_term_goal, closest_goal_map, replan, stop, closest_goal_pt, d2g
    ...
    def _check_collision(self):
        ...
        if dist < self.collision_threshold:
            ...
            print("===> DiscretePlanner: Robot collided!")

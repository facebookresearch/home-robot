...
def load_vc(model_name, step=None):
    assert model_name in VC1_CONFIG_PATHS, f"Model {model_name} not found. Model must be a file in minigpt4/configs/models/vc/*.yaml. Available models : {list(VC1_CONFIG_PATHS.keys())}"
    ...
    def encode_image(self, images):
        with torch.no_grad():
            if self.encoder_only:
                assert hasattr(self._mtm_model._model, "contrastive_image_encoder"), f"Model set to use encoder only but model does not have proper contrastive loss"
                code = self._mtm_model(images)
                code = self._mtm_model._model.contrastive_image_encoder(code)
            else :
                _input = {"images":images.unsqueeze(1), "label":["foo"] * images.shape[0]} # the unsqueeze is for the sequence length of 1
                ...
                _masks = {"label":torch.zeros(1).to(self.device), "images":torch.ones(_input["images"].shape[2]).to(self.device)}
                decoded_trajs = self._mtm_model._model.forward(trajectories=_input, masks = _masks)

...
class GroundedMobileSAM(PerceptionModule):
    def __init__(self, config_file=None, custom_vocabulary: List[str]= "['', 'dog', 'grass', 'sky']", checkpoint_file=MOBILE_SAM_CHECKPOINT_PATH):
    ...
    def predict(self, obs: Observations, depth_threshold: Optional=None):
        ...
        detections.mask = self.segment(
            image=cv2.cvtColor(obs.rgb, cv2.COLOR_BGR2RGB),
            xyxy=detections.xyxy
        )
        semantic_map, instance_map = overlay_masks(detections.mask, detections.class_id, (height, width))
    ...
    def read_images_from_folder(self, folder_name: str):
        ...
        gt_object_mask = cv2.imread(os.path.join(folder_name, "obj_gt.png"), cv2.IMREAD_GRAYSCALE) // 255

        # load groundtruth recetacle segmentation
        gt_recep_mask = cv2.imread(os.path.join(folder_name, "recep_seg_gt.png"), cv2.IMREAD_GRAYSCALE) // 255
        return rgb, gt_object_mask, gt_recep_mask, object_name, start_recep

    def compute_iou(self, gt_object_mask, pred_mask):
        return np.maximum(np.sum(gt_object_mask * pred_mask) / np.sum(gt_object_mask + pred_mask - (gt_object_mask * pred_mask)), 0)


    def save_observations(self, obs, folder_name, method_name: str="grounded_mobile_sam"):
        """
        ...
        cv2.imwrite(os.path.join(folder_name, method_name, "obj.png"), object_seg.astype(np.uint8) * 255)

        recep_seg = obs.semantic == 2

        # save receptacle segmentation
        cv2.imwrite(os.path.join(folder_name, method_name, "rec.png"), recep_seg.astype(np.uint8) * 255)


    def load_segmentations_for_method(self, folder_name: str, method_name: str="grounded_mobile_sam"):
        obj_seg = cv2.imread(os.path.join(folder_name, method_name, 'obj.png'), cv2.IMREAD_GRAYSCALE) // 255 
        recep_seg = cv2.imread(os.path.join(folder_name, method_name, 'rec.png'), cv2.IMREAD_GRAYSCALE) // 255
        return obj_seg, recep_seg

    def quantitative_evaluate(self, folder_name: str, method_name: str="grounded_mobile_sam"):
        rgb, gt_object_mask, gt_recep_mask, object_name,  start_recep = self.read_images_from_folder(folder_name)
        obj_seg, recep_seg = self.load_segmentations_for_method(folder_name, method_name=method_name)
    ...
    def run_single_evaluation(self, folder_name: str, method_name: str="grounded_mobile_sam", save_observations=False):
        """
    ...
    def evaluate_folder(self, folder_name: str, method_name: str="grounded_mobile_sam"):
        time_start = time.time()
        processed = 0
        dfs = []
        for sub_folder in os.listdir(folder_name):
            success = self.run_single_evaluation(os.path.join(folder_name, sub_folder), method_name=method_name)
            time_end = time.time()
        ...
        print('object_iou', 'recep_iou', 'object_detected?', 'recep_detected?', 'false_object_pixels', 'false_recep_pixels')
        print(df['object_iou'].mean(), df['recep_iou'].mean(), df['object_detected?'].mean(), df['recep_detected?'].mean(), df['false_object'].mean(), df['false_recep'].mean())
        return df

if __name__ == "__main__":
    perception_module = GroundedMobileSAM()
    perception_module.evaluate_folder('/srv/flash1/syenamandra3/object-rearrangement/ovmm/home-robot/benchmarks/angle_52_ternary/', method_name='grounded_mobile_sam_unary')
    perception_module.evaluate_folder('/srv/flash1/syenamandra3/object-rearrangement/ovmm/home-robot/benchmarks/angle_52_ternary/', method_name='grounded_mobile_sam_binary')
    perception_module.evaluate_folder('/srv/flash1/syenamandra3/object-rearrangement/ovmm/home-robot/benchmarks/angle_52_ternary/', method_name='detic_ternary')

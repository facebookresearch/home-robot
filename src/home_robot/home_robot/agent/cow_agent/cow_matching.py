...
## DS: NOTE using InstanceMemory class with minor modifications

matplotlib.use("Agg")


class CowMatching(Matching):
    def __init__(
        self,
        device: int,
        score_func: str,
    ...
    def get_matches_against_memory(
        ...
        **kwargs,
        ...
        use_full_image = True ## DS: NOTE hard set for CoW
    ...
    @torch.no_grad()
    def match_image_to_image_clip(self, views_orig, image_goal, **kwargs):
        """Compute matching scores from a language goal to images."""
        batch_size = 64

        image_goal = torch.unsqueeze(self.clip_preprocess(ToPILImage()(image_goal.astype(np.uint8))), dim=0)
        image_goal = self.clip_model.encode_image(image_goal.to(self.device))
        ...
        image_goal = image_goal / image_goal.norm(dim=-1, keepdim=True)
        # compute cosines similarity
        similarity = (image_goal @ view_embeddings.T).squeeze(0)

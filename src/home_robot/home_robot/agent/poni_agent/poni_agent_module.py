...
from home_robot.navigation_policy.object_navigation.objectnav_poni_policy import (
    ObjectNavPONIPolicy,
)
...
def annotate_image(image, text):
    ...
    thickness = 2
    image = cv2.putText(
        image, text, (10, 30), font, fontScale, color, thickness, cv2.LINE_AA
    )
    # Draw border
...
class PONIAgentModule(nn.Module):
    def __init__(self, config, device_id):
        ...
        self.policy = ObjectNavPONIPolicy(
            config.AGENT.PONI.pf_model_path,
            area_weight_coef=config.AGENT.PONI.area_weight_coef,
            pf_masking_opt=config.AGENT.PONI.pf_masking_opt,
            mask_nearest_locations=config.AGENT.PONI.mask_nearest_locations,
            device=self.device,
        )
    ...
    def forward(
        ...
        goal_map, found_goal = self.policy(
            map_features.float().to(self.device),
            seq_object_goal_category.float().to(self.device),
        )
        ...
        cached_vis = self.policy._cached_visualizations[0]
        pfs_vis = []
        for cached_vis in self.policy._cached_visualizations:
            pfs_vis.append(
                np.concatenate(
                    [
                        annotate_image(cached_vis["pfs"], "Overall PF"),
                        annotate_image(cached_vis["area_pfs"], "Area PF"),
                        annotate_image(cached_vis["raw_pfs"], "Object PF"),
                    ],
                    axis=1,
                )
            )

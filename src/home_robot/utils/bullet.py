...
class PbObject(object):
    def __init__(self, name, filename, assets_path=None, start_pos=[0,0,0], start_rot=[0,0,0,1],
                 static=False, client=None):
        self.name = name
...
class PbArticulatedObject(PbObject):
    def __init__(self, name, filename, assets_path=None, start_pos=[0,0,0], start_rot=[0,0,0,1],
                 static=False, client=None, *args, **kwargs):
        super(PbArticulatedObject, self).__init__(name, filename, assets_path, start_pos,
                                                  start_rot, static, client)
        self._link_idx = {}
    ...
    def set_joint_position(self, idx, pos):
        pb.resetJointState(self.id, idx, targetValue=pos, targetVelocity=0.,
                          physicsClientId=self.client)

    def get_link_pose(self, name):
        """ get link pose - forward kinematrics """
        res = pb.getLinkState(self.id, self._link_idx[name], computeForwardKinematics=1, physicsClientId=self.client)
...
class PbCamera(Camera):
    ...
    def __init__(self, client, pos, orn, height=200, width=200, near_val=0.001, far_val=1000., fov=90):
        self.client = client
        self.height = height
        self.width = width
        ...
        self.fov = fov
        self.proj_matrix = pb.computeProjectionMatrixFOV(self.fov, self.width / self.height,
    ...
    def _set_params(self):
        ...
        e = 1 / (np.tan(np.radians(self.fov/2.)))
        t = self.near_val / e; b = -t
    ...
    def capture(self):
        res = pb.getCameraImage(self.width, self.height, self.view_matrix, self.proj_matrix, 
    ...
    def show(self, images=False, show_pc=True, test_id=2):
        ...
        if images:
            ...
            plt.subplot(223); plt.imshow(seg)
            plt.subplot(224); plt.imshow(xyz)
            plt.show()
        xyz = xyz.reshape(-1, 3)
        rgb = rgb.reshape(-1, 3)
        seg = seg.reshape(-1)

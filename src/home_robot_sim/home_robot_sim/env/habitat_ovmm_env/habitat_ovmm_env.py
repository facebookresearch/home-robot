...
class HabitatOpenVocabManipEnv(HabitatEnv):
    ...
    def __init__(self, habitat_env: habitat.core.env.Env, config, dataset):
        ...
        self._instance_ids_start_in_panoptic = config.habitat.simulator.instance_ids_start
        self._rec_id_to_name_mapping = {
            k: v for v, k in self._rec_name_to_id_mapping.items()
        }
        self.store_all_categories_in_map = getattr(config.AGENT.SEMANTIC_MAP, 'store_all_categories', False)
        self.evaluate_instance_tracking = getattr(config.AGENT.SEMANTIC_MAP, 'evaluate_instance_tracking', False)
    ...
    def _preprocess_obs(
        ...
        gt_instance_ids = np.maximum(0, habitat_obs["robot_head_panoptic"] - self._instance_ids_start_in_panoptic + 1)[..., 0]
    ...
    def _preprocess_goal(
        ...
        if self.store_all_categories_in_map:
            ...
            start_rec_goal_id = self._rec_name_to_id_mapping[start_receptacle] + num_obj_categories
            end_rec_goal_id = self._rec_name_to_id_mapping[goal_receptacle] + num_obj_categories

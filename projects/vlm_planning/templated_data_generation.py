    ...
    for _ in range(num_pos_data_per_task):
        ...
        if is_creative_task:
            ...
            context_length = random.randrange(
                num_task_objects+1, max_context_length)
            for _ in range(context_length-num_task_objects-1):
                context.append(get_object_image_lvis(
                    random.sample(irr_vocab, 1)[0], vocab))
            recep_id = get_object_image_lvis(
                predefined_receps[task_id], vocab)

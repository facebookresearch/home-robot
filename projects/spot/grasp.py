...
class GraspController:
    ...
    def grasp(self, image_responses, timeout=10, count=3):
        ...
        if pixels is None:
            print(f" > Could not find object in images; owl-vit failed.")
            continue

        print(f" > Grasping {self.labels} at {pixels}")
        k = 0
        while True:
            ...
            if k == count:
                ...
                if retry == "y":
                    ...
                    # Set k to zero so that we can check this
                    k = 0
                    continue
                else:
                    break
...
def main(show: bool = True):
    ...
    gaze = GraspController(
        spot=spot,
        objects=[["lion plush", "zebra plush", "apple macbook", "bottle of water"]],
        confidence=0.05,
        show_img=show,
        top_grasp=False,
        hor_grasp=True,

...
class StretchDemoBaseEnv(gym.Env):
    ...
    def convert_ros_pose_to_pinocchio(self, joint_angles):  # TODO: the responsibility of robot.py, not this class
        ...
        pin_compatible_joints[1] = pin_compatible_joints[2] = pin_compatible_joints[3] = pin_compatible_joints[4] = \
    ...
    def gripper_ik(self, model, pos, rot, current_joints):
        ...
        ros_pose = self.convert_pinocchio_pose_to_ros(pose.copy())  # TODO: copying just to be safe...
    ...
    def construct_observation(self, image, depth, pose, color_camera_info, depth_camera_info, camera_pose,
        ...
        adjusted_pose = np.concatenate((ee_pos, ee_rot, np.array([gripper_pose, time_fraction])),
                                       axis=-1)  # TODO: trying to remove the base pose+theta (TODO: don't hardcode)
        if camera_info_in_state:
            # TODO: depth and color current have the same intrinsics, so I'm just passing color along for now
            color_camera_state = np.concatenate((color_camera_info["D"],
                                                 color_camera_info["K"].flatten(),
                                                 color_camera_info["R"].flatten(),
                                                 color_camera_info["P"].flatten()))
            camera_pose = camera_pose.flatten()
            adjusted_pose = np.concatenate((adjusted_pose, color_camera_state, camera_pose))

        depth = np.clip(depth, a_min=None, a_max=CAMERA_FAR_PLANE * CAMERA_SCALE_CONST)
        depth_scale = 2 ** 8 / (CAMERA_SCALE_CONST * CAMERA_FAR_PLANE)  # Rescale to get the most we can out of our bits
        depth = depth * depth_scale

        # Convert from uint16 to uint8 (2**8/2**16 = 1/2**8)
        combined_image = np.concatenate((image, np.expand_dims(depth, axis=-1)), axis=-1).astype(np.uint8)

        if context_observation is not None:
            combined_image = np.concatenate((context_observation['image'], combined_image), axis=-1)
            adjusted_pose = np.concatenate((context_observation['state_vector'], adjusted_pose), axis=-1)
    ...
    def get_stretch_obs_and_action_space(self, camera_info_in_state):
        ...
        observation_space = gym.spaces.Dict(
            {"image": gym.spaces.Box(low=0, high=255, shape=(640, 480, channels), dtype=np.uint8),
    ...
    def construct_camera_data_from_robot(self, robot):
        ...
        depth_camera_info = {"D": robot.dpt_cam.D, "K": robot.dpt_cam.K, "R": robot.dpt_cam.R, "P": robot.dpt_cam.P}
        camera_pose = robot.get_pose(robot.rgb_cam.get_frame(), timeout_s=10.0)
        return color_camera_info, depth_camera_info, camera_pose

    def get_images_from_robot(self, robot):
        rgb, depth = robot.get_images(filter_depth=False, compute_xyz=False)  # TODO: filter depth?
        depth = (depth * CAMERA_SCALE_CONST).astype(np.uint16)  # TODO: consistent with recorder
        return rgb, depth
